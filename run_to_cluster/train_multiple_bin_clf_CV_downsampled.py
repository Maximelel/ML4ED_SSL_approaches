# py file to run multiple binary classification with Cross-Validation (CV) and Learning Curves (LC) on CEReD dataset
...
def main(batch_size, epochs, epochs_eval, n_splits, topN, cut_downsampling_train, cut_downsampling_test):
    ...
    train_dataset_downsampled = downsample_dataset(train_dataset, cut_downsampling_train)
    test_dataset_downsampled = downsample_dataset(original_test_dataset, cut_downsampling_test)

    # Check lengths of sets
    print("Train set length:", len(train_dataset_downsampled))
    print("Test set length:", len(test_dataset_downsampled))
    ...
    print(f"cut_downsampling_train = {cut_downsampling_train}")
    print(f"cut_downsampling_test = {cut_downsampling_test}")
    ##############################
    ...
    for i, cat in enumerate(reflective_categories):
        ...
        df_sentences_bin = train_dataset_downsampled.copy()
    ...
    # Load test dataset
    df_sentences_test = test_dataset_downsampled.copy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cut_downsampling_train', type=int, default=500, help='Number of sentences per class in train set')
    parser.add_argument('--cut_downsampling_test', type=int, default=150, help='Number of sentences per class in test set')
    args = parser.parse_args()
    main(args.batch_size, args.epochs, args.epochs_eval, args.n_splits, args.topN, args.cut_downsampling_train, args.cut_downsampling_test)
